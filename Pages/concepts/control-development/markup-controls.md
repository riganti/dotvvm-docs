...
Then, the control can be used in the page like as `<cc:AddressEditor DataContext={value: ShippingAddress} />`
...
If your markup control doesn't depend on a specific viewmodel, use `@viewModel object`. It means that the control can be used in any binding context.

## Properties

The markup control can specify any number of control properties, for passing additional information aside of the `DataContext`.
Properties may contain static values, value binding or commands and are used as HTML attributes.
